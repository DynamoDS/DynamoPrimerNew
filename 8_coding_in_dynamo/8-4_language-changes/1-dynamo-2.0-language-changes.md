...
list@level の新しい構文では、`list@-1` の代わりに `list@L1` を使用します。 ![](../../.gitbook/assets/lang2_1.png)
...
ユーザが最初のノードをキャンバスにドロップし、ジオメトリのリストを接続した場合、複製が始まるものと期待しますが、実際には複製は発生しません。次に示すように、実行時には代わりに 2 番目のオーバーロードが呼び出されるためです。![](../../.gitbook/assets/lang2_2.png)
...
**B.2 つ目の問題: インスタンス メソッドが異種の配列では機能しない**
...
Dynamo は一般的に関数のオーバーロードをサポートしているため、同じ数のパラメータを持つ、別のオーバーロード関数が存在すると、混乱する可能性があります。たとえば、次のグラフでは、`Curve.Extrude` の `direction` 入力に数値を接続し、`Curve.Extrude` の `distance` 入力にベクトルを接続すると、両方のノードが引き続き機能しますが、これは予期しない挙動です。この場合、ノードが静的メソッドにコンパイルされても、エンジンは実行時に違いを見分けることができず、入力型に応じてどちらかを選択します。 ![](../../.gitbook/assets/lang2_6.png)
...
**変数の再定義を防止することで、CBN で自動調整更新が無効になっています。** ![](../../.gitbook/assets/lang2_17.png)
...
**1\..NET ディクショナリを返す Zero-Touch ノードは、Dynamo ディクショナリとして返される**

**IDictionary を返す次の Zero-Touch C# メソッドについて考えてみましょう。** ![](../../.gitbook/assets/lang2_21.png)

**対応する ZT ノードの戻り値が Dynamo ディクショナリとしてマーシャリングされます。** ![](../../.gitbook/assets/lang2_22.png)

**2\.Multi-Return ノードはディクショナリとしてプレビューされる**

**Multi-Return 属性を使用して IDictionary を返す Zero-Touch ノードが Dynamo ディクショナリを返します。** ![](../../.gitbook/assets/lang2_23.png)

![](../../.gitbook/assets/lang2_24.png)

**3\.Dynamo ディクショナリは、.NET ディクショナリを受け入れる Zero-touch ノードに入力として渡すことができる**

**IDictionary パラメータを持つ ZT メソッド:** ![](../../.gitbook/assets/lang2_25.png)

**ZT ノードは、入力として Dynamo ディクショナリを受け入れます。** ![](../../.gitbook/assets/lang2_26.png)
...
また、Code Block ノードには出力ポート情報(Multi-Return 属性の形式)が存在しないため、Code Block のプレビューは UI ノードとは異なり順序付けされません。![](../../.gitbook/assets/lang2_27.png)
