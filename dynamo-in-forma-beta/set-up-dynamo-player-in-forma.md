...
Возможности Dynamo в Forma в настоящее время доступны в виде открытой бета-версии раннего доступа, а это значит, что функции и пользовательский интерфейс могут часто меняться.
...
Для использования классического приложения Dynamo потребуется приложение Dynamo в виде однопользовательской песочницы либо с подключением к Revit или Civil 3D. Кроме того, потребуется пакет DynamoForma.

#### Revit

Следуйте этим инструкциям для настройки Dynamo в Revit и пакета DynamoForma.
...
3. В Dynamo установите пакет DynamoForma. Перейдите в раздел «Пакеты» > Package Manager, затем найдите DynamoForma.
   1. Если используется Revit 2024, установите пакет DynamoForma для выпуска 2.x.
   2. Если установлена версия Revit 2025, установите пакет DynamoForma.

#### Civil 3D

Следуйте этим инструкциям для настройки Dynamo в Civil 3D и пакета DynamoForma.
...
3. В Dynamo установите пакет DynamoForma. Перейдите в раздел «Пакеты» > Package Manager, затем найдите DynamoForma.
   1. Если используется Civil 3D 2024, установите пакет DynamoForma для выпуска 2.x.
   2. Если используется Civil 3D 2025, установите пакет DynamoForma.

#### Dynamo Sandbox

Следуйте этим инструкциям для установки Dynamo Sandbox и пакета DynamoForma.
...
4. В Dynamo установите пакет DynamoForma. Перейдите в раздел «Пакеты» > Package Manager, затем найдите DynamoForma.
   1. Если используется Dynamo 2.x, установите пакет DynamoForma для выпуска 2.x.
   2. Если используется Dynamo 3.x, установите пакет DynamoForma.
