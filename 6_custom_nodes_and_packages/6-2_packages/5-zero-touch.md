...
Zero-Touch 匯入係指匯入 C# 資源庫所用的簡易點選方法。Dynamo 將讀取 _.dll_ 檔案的公開方法，並將其轉換為 Dynamo 節點。您可以使用 Zero-Touch 開發自己的自訂節點與套件，並將外部資源庫匯入至 Dynamo 環境。
...
Zero-touch 套件是使用者定義之自訂節點的良好補充。以下表格中列示了使用 C# 資源庫的一些套件。如需有關套件的更多詳細資訊，請造訪附錄中的[「套件」一節](../../a\_appendix/a-3\_packages.md)。
...
> 4. 最後，我們來查看結果！將 **Watch Image** 節點放在圖元區上，並連接至 **Image.ReadFromFile**。我們尚未使用 AForge，但已成功將影像匯入至 Dynamo。
...
後續步驟將提供熟練使用 [AForge Imaging API](http://www.aforgenet.com/framework/docs/html/d087503e-77da-dc47-0e33-788275035a90.htm) 的某些技巧。您不需要瞭解 Dynamo 工作的所有這些技巧。這更多是展示在 Dynamo 環境中靈活使用外部資源庫。
...
> 2. 使用 Code Block，我們將資料組織為適合 Rectangle.ByCornerPoints 節點 (以下程式碼) 的結構。
...
我們現在有一系列代表影像中白色正方形的矩形。透過程式設計，我們已實現與 Illustrator 中的即時追蹤 (大致) 類似的功能！

不過，我們還需要進行一些清理。拉近後，可以看到有一些不需要的小矩形。

![](<../images/6-2/5/case study aforge exercise 2 - 08.jpg>)

接下來，我們將編寫程式碼以除去不需要的矩形。
...
最後，將 both\_sides 輸入變更為 False，會產生一個方向的擠出。將產生的此結果浸入樹脂，將產生造型極為奇特的桌子。

![](<../images/6-2/5/case study aforge exercise 2 - 11.jpg>)

以上是基本範例，但其中展示的概念通用於激動人心的真實應用。電腦視覺可用於許多程序。這裡列示一些範例：條碼讀取器、透視配合、[投影對映](https://www.youtube.com/watch?v=XSR0Xady02o)及[增強實景](http://aforgenet.com/aforge/articles/gratf\_ar/)。如需此練習相關的更多 AForge 進階主題，請參閱[本文](http://aforgenet.com/articles/shape\_checker/)。
