...
_Примечание. Создавать Dynamo вручную до использования DynamoRevit требуется только при сборке Dynamo 1.x и DynamoRevit 1.x. Новые версии репозитория DynamoRevit используют диспетчер пакетов NuGet для загрузки необходимых зависимостей Dynamo. Хотя для сборки DynamoRevit 2.x не нужно извлекать Dynamo вручную, для запуска `надстройки` DynamoRevit потребуются файлы Core `DLL`, так что мы рекомендуем все равно загрузить и собрать Dynamo. См. подробности ниже:_ [_Создание репозитория с помощью Visual Studio_](1-build-dynamorevit-from-source.md#building-the-repository-using-Visual-Studio).
...
![Интерфейс командной строки](../../.gitbook/assets/cli-cd-revit.jpg)
...
![Интерфейс командной строки после клонирования репозитория](../../.gitbook/assets/cli-clone-revit.jpg)
...
![Каталог переключен на ветвь](../../.gitbook/assets/cli-branch-revit.jpg)
...
Перед созданием репозитория необходимо восстановить пакеты NuGet с файлом `restorepackages.bat` в папке `src`. В этом файле BAT используется диспетчер пакетов [NuGet](https://www.nuget.org) для извлечения встроенных двоичных файлов Dynamo Core, необходимых для работы DynamoRevit. Их также можно создать вручную, но только если изменения вносятся в DynamoRevit, а не в ядро Dynamo. Это позволяет быстрее начать работу. Убедитесь, что этот файл запущен от имени администратора.

![Запуск от имени администратора](../../.gitbook/assets/fe-restorepackages.jpg)
...
![Последние бета-версии пакетов Dynamo NuGet](../../.gitbook/assets/fe-packages.jpg)
...
![Сборка решения](../../.gitbook/assets/vs-build-dynamorevit.jpg)
...
Кроме того, необходимо удалить существующее приложение Dynamo, входящее в комплект поставки Revit. Для этого перейдите в каталог `C:\\Program Files\Autodesk\Revit 2023\AddIns` и удалите две папки, в названии которых есть текст **Dynamo**: `DynamoForRevit` и `DynamoPlayerForRevit`. Их можно удалить или создать резервную копию в отдельной папке, если требуется восстановить исходное приложение Dynamo for Revit.

![Папки DynamoForRevit и DynamoPlayerforRevit](../../.gitbook/assets/fe-dynamo-folders-remove.jpg)
...
![Надстройка Dynamo, расположенная на вкладке «Управление»](../../.gitbook/assets/revit-dynamo.jpg)
...
![Узел Wall.ByCurveAndHeight вызвал исключение](../../.gitbook/assets/dyn-wallbycurveandheight.jpg)
...
![Окно «Присоединить к процессу»](../../.gitbook/assets/vs-debug-attachprocess.jpg)
...
![Установка точки прерывания](../../.gitbook/assets/vs-debugging-breakpoint.jpg)
...
![Достигнута точка прерывания в Visual Studio](../../.gitbook/assets/vs-breakpoint.jpg)
...
![Шаг с обходом в Visual Studio](../../.gitbook/assets/vs-stepover.jpg)
...
Поскольку это не библиотека с открытым исходным кодом, мы не можем вносить в нее изменения. Имея дополнительную информацию, мы можем описать [проблему](https://guides.github.com/features/issues/) с контекстом на GitHub или предложить временное решение проблемы, используя запрос на слияние.

![Исключение в Visual Studio](../../.gitbook/assets/vs-exception.jpg)
...
![Задание каталога в интерфейсе командной строки](../../.gitbook/assets/cli-pull-revit.jpg)
