...
* Ao corrigir um erro, adicione um teste de unidade que destaque como o comportamento atual é quebrado.
...
Ao enviar uma solicitação de extração, use o [modelo PR padrão](https://github.com/DynamoDS/Dynamo/blob/master/.github/PULL\_REQUEST\_TEMPLATE.md). Antes de enviar a solicitação de extração, certifique-se de que o propósito esteja claramente descrito e que todas as declarações possam ser afirmadas como verdadeiras:

* A base de código está em um estado melhor após essa solicitação de extração.
* A documentação está de acordo com os [padrões](https://github.com/DynamoDS/Dynamo/wiki/Coding-Standards).
* O nível de teste que essa solicitação de extração inclui é apropriado.
* Sequências de caracteres voltadas para o usuário, se existirem, serão extraídas para arquivos `*.resx`.
* Todos os testes passam usando a Integração contínua de autoatendimento.
...
* As solicitações de extração devem usar o modelo de solicitação de extração padrão do Dynamo.
