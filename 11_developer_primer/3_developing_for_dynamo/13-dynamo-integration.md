...
* [Dynamo Revit Selection 節點](13-dynamo-integration.md#dynamo-revit-selection-nodes-what-are-they) 如何實作節點，讓使用者選取主體中的物件或資料，並將其作為輸入傳入 Dynamo 圖表。
...
* Callsite
  * 可執行檔包含多個 Callsite。這些 Callsite 用於將執行作業分派到需要分派的各個位置：
    ...
    * 自訂節點 (DS 函數)
* TraceSerializer
  * 將資料的序列化和還原序列化處理到追蹤中。此機制按照一個多步驟程序處理追蹤資料：
    * 儲存圖表 (序列化) 時，特定節點產生的資料會轉換為結構化 JSON 字串。此序列化資料使用 gzip 進行壓縮以減少檔案大小，然後使用 Base64 編碼確保安全儲存。產生的安全字串會嵌入 .dyn 檔案的 bindings 區段。
    * 開啟圖表 (還原序列化) 時，則會從 .dyn 檔案的 bindings 區段讀取編碼的字串。該字串使用 Base64 解碼回其原始壓縮的二進位形式，然後使用 gzip 解壓縮。還原的結構化資料會轉換回作用中的執行階段物件。
...
不要依賴 .dyn 檔案中資料的序列化格式。序列化的資料不應直接存取，只能透過追蹤 API 完成。
...
每個 `TraceExampleItem` 都儲存在靜態的 TraceableObjectManager 中。TraceableObjectManager 會將 Dynamo 的追蹤記錄序列化和還原序列化為 JSON 字串。因此，節點可以找到並更新先前建立的元素，而不是重新建立。
...
TraceableId 類別是針對我們希望儲存到追蹤中的每個 `TraceExampleItem` 而建立、經過序列化、base64 編碼，並在圖表儲存時儲存到磁碟，讓繫結可以重新建立關聯，即使之後基於既有元素字典重新開啟圖表時也是如此。在此範例中無法運作得很好，因為字典並不像 Revit 文件一樣持久。
...
* [NodeModel 案例研究 - 自訂使用者介面](5-nodemodel-case-study-custom-ui.md)
* [針對 Dynamo 2.x 更新您的套件和 Dynamo 資源庫](6-0-updating-your-packages-and-dynamo-libraries-for-dynamo-2x.md)
* [針對 Dynamo 3.x 更新套件與 Dynamo 資料庫](6-1-updating-your-packages-and-dynamo-libraries-for-dynamo-3x-Net8.md)
* [針對 Dynamo 4.x 更新套件與 Dynamo 資料庫](6-2-updating-your-packages-and-dynamo-libraries-for-dynamo-4x.md)
