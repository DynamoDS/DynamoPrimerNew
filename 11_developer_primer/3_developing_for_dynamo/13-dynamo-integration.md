...
* [Dynamo Revit 选择节点](13-dynamo-integration.md#dynamo-revit-selection-nodes-what-are-they) 如何实现允许用户从主机中选择对象或数据并将其作为输入传递到 Dynamo 图表的节点。
...
* 调用站点
  * 可执行文件包含多个调用站点。这些调用站点用于将执行分派到需要从中分派执行的各个位置：
    ...
    * 自定义节点（DS 功能）
* TraceSerializer
  * 将数据的序列化和反序列化处理到跟踪中。此机制遵循一个多步骤过程来处理跟踪数据：
    * 保存图形（序列化）时，特定节点生成的数据将转换为结构化 JSON 字符串。这些序列化数据使用 gzip 进行压缩以减小文件大小，然后进行 Base64 编码以确保安全存储。生成的安全字符串嵌入到 .dyn 文件的绑定部分中。
    * 打开图形（反序列化）时，将从 .dyn 文件的绑定部分读取编码字符串。该字符串通过 Base64 解码回其原始压缩二进制形式，然后使用 gzip 解压缩。还原的结构化数据将转换回活动运行时对象。
...
不要依赖 .dyn 文件中的数据序列化格式。序列化数据不应直接访问，只能通过跟踪 API 完成。
...
每个 `TraceExampleItem` 存储在静态 TraceableObjectManager 中。TraceableObjectManager 将 Dynamo 的跟踪记录序列化和反序列化为 JSON 字符串。因此，节点可以查找和更新以前创建的元素，而不是重新创建它们。
...
TraceableId 类是为我们希望保存到跟踪中的每个 `TraceExampleItem` 创建的，经过序列化、base64 编码并在保存图形时保存到磁盘，以便可以重新关联绑定，甚至以后在现有元素字典之上重新打开图形时也是如此。在此示例中，这行不通，因为字典不像 Revit 文档那样是持久的。
...
* 目前，每个跟踪字符串都使用 gzip 压缩和 base64 编码进行序列化。这是对旧版 SOAP xml 格式的改进。
...
* [NodeModel 案例研究 - 自定义 UI](5-nodemodel-case-study-custom-ui.md)
* [更新 Dynamo 2.x 的软件包和 Dynamo 库](6-0-updating-your-packages-and-dynamo-libraries-for-dynamo-2x.md)
* [更新 Dynamo 3.x 的软件包和 Dynamo 库](6-1-updating-your-packages-and-dynamo-libraries-for-dynamo-3x-Net8.md)
* [更新 Dynamo 4.x 的软件包和 Dynamo 库](6-2-updating-your-packages-and-dynamo-libraries-for-dynamo-4x.md)
