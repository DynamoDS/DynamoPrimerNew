...
* [Tento úvod](13-dynamo-integration.md#dynamo-integration) Základní přehled toho, co tato příručka obsahuje a o čem je aplikace Dynamo.
* [Vlastní vstupní bod aplikace Dynamo](13-dynamo-integration.md#dynamo-custom-entry-point) Jak vytvořit DynamoModel a kde začít.
* [Vázání a trasování prvků](13-dynamo-integration.md#element-binding-and-trace) Použití mechanismu trasování aplikace Dynamo ke svázání uzlů v grafu s jejich výsledky v hostiteli.
* [Uzly Selection doplňku Dynamo Revit ](13-dynamo-integration.md#dynamo-revit-selection-nodes-what-are-they) Jak implementovat uzly, které uživatelům umožňují vybírat objekty nebo data z hostitele a předávat je jako vstupy do grafu Dynamo.
* [Přehled vestavěných balíčků aplikace Dynamo](13-dynamo-integration.md#dynamo-built-in-packages-overview) Co je standardní knihovna aplikace Dynamo a jak používat základní mechanismus k dodávání balíčků s integrací.
...
_Trasování_ je mechanismus v jádře aplikace Dynamo, který dokáže serializovat data do souboru .dyn (soubor aplikace Dynamo). Důležité je, že tato data jsou klíčová pro místa volání uzlů v grafu aplikace Dynamo.
...
* Serializátor trasování (TraceSerializer)
  * Zpracovává serializaci a deserializaci dat do trasování. Tento mechanismus využívá vícekrokový proces zpracování dat trasování:
    * Při ukládání grafu (serializace) se data generovaná konkrétními uzly převedou na strukturovaný řetězec JSON. Tato serializovaná data jsou následně komprimována pomocí gzip, aby se zmenšila velikost souboru, a jsou zakódována ve formátu Base64, aby se zajistilo bezpečné uložení. Výsledný bezpečný řetězec je vložen do sekce vazeb souboru .dyn.
    * Při otevírání grafu (deserializace) se zakódovaný řetězec načte z části vazeb souboru .dyn. Řetězec je pomocí Base64 dekódován zpět do původní komprimované binární podoby a poté dekomprimován pomocí gzip. Obnovená strukturovaná data jsou poté převedena zpět na aktivní objekty modulu runtime.
...
Později, když uživatel upraví vstup ve svém programu a znovu jej spustí, jeho program bude _(bez vazby prvků)_ vytvoří 100 nových dveří, přičemž staré dveře budou v modelu stále existovat spolu s novými.
...
Zjistili jsme, že to obvykle není to, co uživatelé očekávají. Místo toho se při povolené vazbě prvků předchozí výsledky spuštění grafu vyčistí a odstraní nebo upraví. Která z těchto akcí (_odstranění nebo úprava_) proběhne, závisí na flexibilitě rozhraní API hostitele. Při povolené vazbě prvků bude po druhém, třetím nebo padesátém spuštění uživatelského programu model aplikace Dynamo obsahovat stále pouze 100 dveří.
...
Nespoléhejte se na serializovaný formát dat v souboru .dyn. Serializovaná data by neměla být přístupná přímo, lze je zpracovávat pouze prostřednictvím rozhraní API pro trasování.

Vazba prvků je naproti tomu vytvořena nad rozhraními API pro trasování a je implementována v integraci aplikace Dynamo _(DynamoRevit, Dynamo4Civil atd.)_
...
Každý objekt `TraceExampleItem` je uložen ve statickém objektu TraceableObjectManager. TraceableObjectManager serializuje a deserializuje záznamy trasování aplikace Dynamo jako řetězec JSON. Uzly tak mohou vyhledat a aktualizovat dříve vytvořené prvky, místo toho, aby je znovu vytvářely.
...
* V současné době je každý řetězec trasování serializován pomocí komprese gzip a kódování base64. Tento přístup představuje zlepšení oproti staršímu formátování SOAP XML.
...
* [Případová studie uzlu NodeModel – vlastní uživatelské rozhraní](5-nodemodel-case-study-custom-ui.md)
* [Aktualizace balíčků a knihoven aplikace Dynamo pro aplikaci Dynamo 2.x](6-0-updating-your-packages-and-dynamo-libraries-for-dynamo-2x.md)
* [Aktualizace balíčků a knihoven aplikace Dynamo pro aplikaci Dynamo 3.x](6-1-updating-your-packages-and-dynamo-libraries-for-dynamo-3x-Net8.md)
* [Aktualizace balíčků a knihoven aplikace Dynamo pro aplikaci Dynamo 4.x](6-2-updating-your-packages-and-dynamo-libraries-for-dynamo-4x.md)
...
S určitými omezeními bude toto umístění použitelné pro klienty a integrátory aplikace Autodesk Dynamo k distribuci jejich balíčků specifických pro integraci. _(Například integrace aplikace Dynamo FormIt vyžaduje vlastní balíček Dynamo FormIt.)_
