...
* [Введение](13-dynamo-integration.md#dynamo-integration): общий обзор этого руководства и программы Dynamo.
* [Настраиваемая точка входа Dynamo](13-dynamo-integration.md#dynamo-custom-entry-point): создание модели Dynamo и начало работы.
* [Привязка и трассировка элементов](13-dynamo-integration.md#element-binding-and-trace): использование механизма трассировки Dynamo для привязки узлов в графе к их результатам в основном приложении.
* [Узлы Selection в Dynamo Revit](13-dynamo-integration.md#dynamo-revit-selection-nodes-what-are-they): реализация узлов, которые позволяют пользователям выбирать объекты или данные из основного приложения и передавать их в качестве входных данных для графа Dynamo.
* [Обзор встроенных пакетов Dynamo](13-dynamo-integration.md#dynamo-built-in-packages-overview): сведения о том, что такое стандартная библиотека Dynamo и как использовать лежащий в ее основе механизм поставки пакетов с интеграцией.
...
Недавно мы добавили для `DynamoSandbox.exe` и `DynamoCLI.exe` возможность загрузки определенной версии ASM. Чтобы пропустить обычный процесс поиска в реестре, можно использовать флаг `--GeometryPath`, чтобы принудительно загрузить ASM Dynamo по определенному пути.
...
_Трассировка_ — это механизм в ядре Dynamo, который позволяет сериализовать данные в файл DYN (Dynamo). Важно отметить, что эти данные привязаны к местам вызова узлов на графе Dynamo.
...
* TraceSerializer
  * Выполняет сериализацию данных в трассировку и десериализацию их из нее. Этот механизм выполняет многошаговый процесс обработки данных трассировки.
    * При сохранении графа (сериализации) данные, созданные определенными узлами, преобразуются в структурированную строку JSON. Эти сериализованные данные сжимаются с помощью утилиты gzip для уменьшения размера файла, а затем кодируются на основе стандарта Base64 для безопасного хранения. Полученная безопасная строка встраивается в раздел привязок файла DYN.
    * При открытии графа (десериализации) закодированная строка считывается из раздела привязок файла DYN. Строка декодируется на основе Base64 обратно в исходную сжатую двоичную форму, а затем распаковывается с помощью gzip. Восстановленные структурированные данные преобразуются обратно в активные объекты среды выполнения.
...
Позже, когда пользователь изменит входные данные в программе и выполнит их повторно, программа _(без привязки элементов)_ создаст 100 новых дверей. При этом старые двери будут присутствовать в модели вместе с новыми.
...
Не полагайтесь на сериализованный формат данных в файле DYN. К сериализованным данным не следует обращаться напрямую, это можно сделать только через API трассировки.

Компонент ElementBinding, напротив, является надстройкой на основе API трассировки и реализован в интеграции Dynamo _(DynamoRevit, Dynamo4Civil и т. д.)_.
...
Каждый элемент `TraceExampleItem` хранится в статическом объекте TraceableObjectManager. TraceableObjectManager сериализует и десериализует записи трассировки Dynamo в виде строки JSON. Таким образом, узлы могут искать и обновлять ранее созданные элементы, а не создавать их повторно.
...
* В настоящее время каждая строка трассировки сериализуется с использованием сжатия gzip и кодировки Base64. Это позволяет добиться лучшего результата по сравнению с устаревшим форматированием XML SOAP.
...
В общих чертах, **эти узлы можно представить как функцию, которая принимает идентификатор элемента и возвращает указатель на этот элемент или какую-либо геометрию, представляющую этот элемент.**
...
* Узлы выбора Selection в `DynamoRevit` реализуются в проекте`RevitUINodes.dll` который ссылается на WPF. Возможно, это не проблема, но об этой особенности стоит помнить при использовании определенных целевых платформ.
...
* [Пример использования NodeModel — настраиваемый пользовательский интерфейс](5-nodemodel-case-study-custom-ui.md)
* [Обновление пакетов и библиотек Dynamo для Dynamo 2.x](6-0-updating-your-packages-and-dynamo-libraries-for-dynamo-2x.md)
* [Обновление пакетов и библиотек Dynamo для Dynamo 3.x](6-1-updating-your-packages-and-dynamo-libraries-for-dynamo-3x-Net8.md)
* [Обновление пакетов и библиотек Dynamo для Dynamo 4.x](6-2-updating-your-packages-and-dynamo-libraries-for-dynamo-4x.md)
...
С некоторыми ограничениями это расположение можно использовать для клиентов и интеграторов ADSK Dynamo в целях распространения пакетов для интеграции. _(Например, для интеграции Dynamo и FormIt требуется настраиваемый пакет Dynamo FormIt.)_
