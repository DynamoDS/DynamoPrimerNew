...
内容：
* [本简介](#dynamo-integration) 对本指南包含的内容以及 Dynamo 内容的高级概述。
* [Dynamo 自定义入口点](#dynamo-custom-entry-point) 如何创建 DynamoModel 以及从何处开始。
* [元素绑定和追踪](#-element-binding-and-trace) 使用 Dynamo 的追踪机制将图形中的节点绑定到其在主机中的结果。
* [Dynamo Revit 选择节点](#-dynamo-revit-selection-nodes) 如何实现允许用户从主机中选择对象或数据并将其作为输入传递到 Dynamo 图表的节点。 
* [Dynamo 内置软件包概述](#dynamo-built-in-packages-overview) 什么是 Dynamo 标准库，以及如何使用基本机制随集成一起交付软件包。
...
最近，我们添加了 `DynamoSandbox.exe` 和 `DynamoCLI.exe` 以加载特定 ASM 版本的功能。要跳过正常的注册表搜索行为，可以使用 `�gp` 标志强制 Dynamo 从特定路径加载 ASM。 
...
*跟踪*是 DynamoCore 中的一种机制，支持将数据序列化到 .dyn（Dynamo 文件）。至关重要的是，此数据对 Dynamo 图形中节点的调用站点具有键控。
...
我们发现这通常不是用户所期望的，相反，在启用元素绑定的情况下，图形执行的先前结果会被清理、删除或修改。哪一种（*删除或修改*）取决于主机 API 的灵活性。启用图元绑定后，在用户 Dynamo 程序的第 2 次、第 3 次或第 50 次运行后 - 模型中只有 100 扇门。
...
DynamoRevit 中有多个 `�Selection�` 节点，我们可以将它们至少分为两组： 
...
2. 文档查询： 
    ...
    这些节点允许用户在整个文档中查询元素子集；这些节点通常返回指向基础 Revit 元素的包装器。这些包装器是 DynamoRevit 体验中不可或缺的一部分，可提供更高级的功能（如图元绑定），并允许 Dynamo 集成器拾取和选择将哪些主机 API 作为节点显示给用户。
...
1. 
    ...
    * 用户放置“Element.Geometry”节点并将 `SelectModelElement` 输出附加到此新节点提取包装墙的几何图形，并使用 libG API 转换为 Dynamo 几何图形。 

    * 用户将图形切换到自动运行模式。 

    * 用户修改 Revit 中的原始墙。 

    * 当 Revit 文档引发事件表明某些图元已更新、选择节点监视此事件并查看其选择的图元的 ID 已修改时，将自动重新运行该图形。 
...
* `DynamoRevit` 中的选择节点在引用 WPF 的 `RevitUINodes.dll` 项目中实现。这可能并非问题但值得注意，具体取决于您的目标平台。 
...
* `BuildOutputAST` 方法的实现 - 此方法需要返回一个 AST，该 AST 将在未来的某个时间点执行该节点。对于 Selection 节点，它应从图元 ID 返回图元或几何图形。https://github.com/DynamoDS/DynamoRevit/blob/master/src/Libraries/RevitNodesUI/Selection.cs#L280 
...
* 文档查询节点更简单一些，因为它们不需要存储对任何图元 ID 的参考，请参见下文以了解 `ElementQueryBase` 类和派生类的实现。执行时，这些节点会调用 Revit API 并在基础文档中查询图元，然后执行前面提到的到几何图形或 Revit 图元包装器的转换。 
...
* [更新 Dynamo 3.x 的软件包和 Dynamo 库](11\_developer\_primer/3\_developing\_for\_dynamo/updating-your-packages-and-dynamo-libraries-for-dynamo-3x-Net8.md)
...
尽管存在一些约束，但此位置将可供 ADSK Dynamo 客户和集成商分发其特定于集成的软件包。*（例如，Dynamo Formit 集成需要自定义的 Dynamo Formit 软件包）。*
