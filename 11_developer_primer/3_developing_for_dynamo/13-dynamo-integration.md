...
* [이 소개](13-dynamo-integration.md#dynamo-integration) 이 안내서에서 다루는 내용을 간략하게 소개하고 Dynamo가 무엇인지 개략적으로 설명합니다.
* [Dynamo 사용자 지정 진입점](13-dynamo-integration.md#dynamo-custom-entry-point) DynamoModel을 작성하는 방법과 시작 위치를 안내합니다.
* [요소 바인딩 및 추적](13-dynamo-integration.md#element-binding-and-trace) Dynamo의 추적 메커니즘을 사용하여 그래프의 노드를 호스트의 결과에 바인딩하는 방법을 설명합니다.
* [Dynamo Revit 선택 노드](13-dynamo-integration.md#dynamo-revit-selection-nodes-what-are-they) 사용자가 호스트에서 객체 또는 데이터를 선택하고 이를 Dynamo 그래프에 입력으로 전달할 수 있는 노드를 구현하는 방법을 안내합니다.
* [Dynamo 기본 제공 패키지 개요](13-dynamo-integration.md#dynamo-built-in-packages-overview) Dynamo 표준 라이브러리란 무엇이며 기본 메커니즘을 사용하여 통합을 포함한 패키지를 제공하는 방법을 설명합니다.
...
_추적_은 데이터를 .dyn(Dynamo 파일)으로 직렬화할 수 있는 Dynamo Core의 메커니즘입니다. 중요한 것은 이 데이터가 Dynamo 그래프 내에서 노드의 Callsite에 연결되어 있다는 점입니다.
...
* Callsite
  * 실행 파일에는 여러 개의 Callsite가 포함되어 있습니다. 이러한 Callsite는 다음에서 디스패치해야 하는 여러 위치로 실행을 디스패치하는 데 사용됩니다.
    ...
    * 사용자 노드(DS 함수)
* TraceSerializer
  * 데이터의 직렬화 및 역직렬화를 추적으로 처리합니다. 이 메커니즘은 추적 데이터를 처리하기 위해 다단계 프로세스를 따릅니다.
    * 그래프를 저장할 때(직렬화) 특정 노드에서 생성된 데이터가 구조화된 JSON 문자열로 변환됩니다. 이 직렬화된 데이터는 파일 크기를 줄이기 위해 gzip으로 압축된 후, 안전한 저장을 위해 Base64로 인코딩됩니다. 생성된 안전한 문자열은 .dyn 파일의 bindings 섹션에 포함됩니다.
    * 그래프를 열 때(역직렬화) .dyn 파일의 bindings 섹션에서 인코딩된 문자열을 읽어옵니다. 이 문자열은 Base64로 디코딩되어 원래 압축된 이진 형식으로 복원되고, gzip으로 압축이 해제됩니다. 복원된 구조화된 데이터는 활성 런타임 객체로 다시 변환됩니다.
...
나중에 사용자가 프로그램에 대한 입력을 수정하고 다시 실행하면 프로그램이 _(요소 바인딩 없이)_ 새 문 100개를 작성하는데, 이전 문은 새 문과 함께 모델에 계속 존재합니다.

***

Dynamo는 라이브 프로그래밍 환경이며 그래프를 변경하면 새로운 실행이 트리거되는 `"Automatic"` 실행 모드가 특징이기 때문에 프로그램 실행 횟수가 많으면 모델이 빠르게 복잡해질 수 있습니다.

이는 일반적으로 사용자가 기대하는 상황이 아닙니다. 대신 요소 바인딩이 활성화된 상태에서는 그래프 실행의 이전 결과가 정리되고 삭제되거나 수정됩니다. 호스트 API의 유연성에 따라 _삭제 또는 수정_됩니다. 요소 바인딩을 사용하면 사용자가 Dynamo 프로그램을 두 번째, 세 번째 또는 50번째 실행한 이후에도 모델에는 문이 100개만 있습니다.
...
.dyn 파일의 직렬화된 데이터 형식에 의존하지 마십시오. 직렬화된 데이터에는 직접 액세스해서는 안 되며 추적 API를 통해서만 액세스할 수 있습니다.

반면 ElementBinding은 추적 API를 기반으로 빌드되며 Dynamo 통합_(DynamoRevit, Dynamo4Civil 등)_에서 구현됩니다.
...
각 `TraceExampleItem`은 정적 TraceableObjectManager에 저장됩니다. TraceableObjectManager는 Dynamo의 추적 기록을 JSON 문자열로 직렬화 및 역직렬화합니다. 이를 통해 노드는 요소를 새로 작성하는 대신, 이전에 작성된 요소를 찾아 업데이트할 수 있습니다.
...
TraceableId 클래스는 추적에 저장하려는 각 `TraceExampleItem`에 대해 작성되며, 이 객체는 직렬화되고 base64 인코딩된 후 그래프가 저장될 때 디스크에 저장됩니다. 이렇게 저장된 정보는 그래프를 나중에 다시 열어, 이미 존재하는 요소 사전 위에서 실행하더라도 바인딩을 다시 연결할 수 있도록 합니다. 사전은 Revit 문서처럼 영구적이지 않으므로 이 예에서는 제대로 작동하지 않습니다.
...
* 현재 각 추적 문자열은 gzip 압축 및 base64 인코딩을 사용하여 직렬화됩니다. 이는 기존의 SOAP xml 형식에 비해 개선된 방식입니다.
...
* [NodeModel 사례 연구 - 사용자 지정 UI](5-nodemodel-case-study-custom-ui.md)
* [Dynamo 2.x용 패키지 및 Dynamo 라이브러리 업데이트하기](6-0-updating-your-packages-and-dynamo-libraries-for-dynamo-2x.md)
* [Dynamo 3.x용 패키지 및 Dynamo 라이브러리 업데이트하기](6-1-updating-your-packages-and-dynamo-libraries-for-dynamo-3x-Net8.md)
* [Dynamo 4.x용 패키지 및 Dynamo 라이브러리 업데이트하기](6-2-updating-your-packages-and-dynamo-libraries-for-dynamo-4x.md)
...
몇 가지 구속조건이 있기는 하지만 이 위치는 ADSK Dynamo 클라이언트 및 통합 개발자가 통합별 패키지를 배포하는 데 사용할 수 있습니다. _(예를 들어, Dynamo Formit 통합을 위해서는 사용자 Dynamo Formit 패키지가 필요합니다.)_
