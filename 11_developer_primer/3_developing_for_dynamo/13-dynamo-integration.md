...
* [Dynamo Revit 選択ノード](13-dynamo-integration.md#dynamo-revit-selection-nodes-what-are-they): ホストからオブジェクトまたはデータを選択し、それらを Dynamo グラフへの入力として渡すことができるノードを実装する方法について説明します。
...
* Callsite
  * 実行可能ファイルには複数の Callsite が含まれています。これらの Callsite は、ディスパッチする必要があるさまざまな場所に実行をディスパッチするために使用されます。
    ...
    * カスタム ノード(DS 関数)
* TraceSerializer
  * トレースへのデータのシリアル化と逆シリアル化を処理します。このメカニズムは、トレース データを処理するための複数ステップのプロセスに従います。
    * グラフを保存する(シリアル化する)場合、特定のノードによって生成されたデータは構造化された JSON 文字列に変換されます。このシリアル化されたデータは、ファイル サイズを縮小するために gzip を使用して圧縮され、安全な保管を確実にするために Base64 でエンコードされます。生成された安全な文字列は、.dyn ファイルのバインド セクションに埋め込まれます。
    * グラフを開く(逆シリアル化する)場合、エンコードされた文字列が .dyn ファイルのバインド セクションから読み取られます。文字列は Base64 でデコードされ、元の圧縮バイナリ形式に戻され、gzip を使用して解凍されます。復元された構造化データは、アクティブなランタイム オブジェクトに変換されます。
...
.dyn ファイル内のデータのシリアル化された形式に依存しないようにします。シリアル化されたデータに直接アクセスすることはできず、トレース API を介してのみ実行できます。
...
それぞれの `TraceExampleItem` は、静的な TraceableObjectManager に格納されます。TraceableObjectManager は、Dynamo のトレース レコードを JSON 文字列としてシリアル化およびシリアル化解除します。そのためノードは、以前に作成された要素を再作成するのではなく、見つけて更新することができます。
...
TraceableId クラスは、トレースに保存する `TraceExampleItem` ごとに作成およびシリアル化され、base64 でエンコードされて、グラフ保存時にディスクに保存されます。これにより、後で既存の要素ディクショナリの上でグラフを再度開いたときにバインドを再度関連付けることができます。この例の場合、ディクショナリは Revit ドキュメントのように実際には永続的ではないため、これはうまく機能しません。
...
* 現在、各トレース文字列は gzip 圧縮と base64 エンコーディングを使用してシリアル化されます。これは、従来の SOAP XML 形式よりも改善されています。
...
* [NodeModel ケース スタディ - カスタム UI](5-nodemodel-case-study-custom-ui.md)
* [Dynamo 2.x 用のパッケージと Dynamo ライブラリを更新する](6-0-updating-your-packages-and-dynamo-libraries-for-dynamo-2x.md)
* [Dynamo 3.x 用のパッケージと Dynamo ライブラリを更新する](6-1-updating-your-packages-and-dynamo-libraries-for-dynamo-3x-Net8.md)
* [Dynamo 4.x 用のパッケージと Dynamo ライブラリを更新する](6-2-updating-your-packages-and-dynamo-libraries-for-dynamo-4x.md)
