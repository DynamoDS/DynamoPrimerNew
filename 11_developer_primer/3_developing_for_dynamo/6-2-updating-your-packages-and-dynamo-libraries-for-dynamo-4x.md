...
Because Dynamo 4.x now runs on the .NET 10 runtime, packages that were built for Dynamo 2.x (_using .NET48_) and Dynamo 3.x(_using .NET 8_) are not guaranteed to work in Dynamo 4.x. When you attempt to download a package in Dynamo 4.x that was published from a Dynamo version less than 4.0, you will get a warning that the package is from an older version of Dynamo.
