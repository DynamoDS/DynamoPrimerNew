...
> 2. 定义另一个 _代码块_，其中列表与参数名称具有相同长度。在本例中，我们命名变量（不带引号），这将为 _代码块创建输入。_ 将 _“滑块”_ 连接到各自的输入：`{bw,bl,bh,ao,io,lu};`
> 3. 将代码块连接到 _“Element.SetParameterByName”\*_ 值输入。在选中“自动运行”后，我们将自动看到结果。
...
> 2. 此外，我们还会创建 _数字滑块_，然后重命名为相应的参数。前三个滑块（从上到下）应重新映射到 \[0,10] 的域，而最后一个滑块（_“外立面弯曲位置”_）应重新映射到 \[0,1] 的域。这些值（从上到下）应从以下这些值开始（尽管它们是任意值）：2.68、2.64、2.29、0.5
