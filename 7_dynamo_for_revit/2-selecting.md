...
_포인트 앤 클릭_ 은 Revit 요소를 직접 선택하는 가장 쉬운 방법입니다. 전체 모델 요소 또는 해당 상의 일부(예: 면 또는 모서리)를 선택할 수 있습니다. 이렇게 하면 해당 Revit 객체에 동적으로 연결된 상태로 유지되므로 Revit 파일이 해당 위치 또는 매개변수를 업데이트하면 참조된 Dynamo 요소가 그래프에서 업데이트됩니다.
...
> 4. _List.GetItemAtIndex_ 를 사용하여 _index_ 에 대해 리스트와 \*integer slider\*를 입력합니다. 선택한 상태에서 슬라이딩하면서 _index 9_ 에서 중지한 후, 트러스를 호스팅하는 주 정면을 분리했습니다.
