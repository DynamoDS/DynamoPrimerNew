...
_“点击”_ 是直接选择 Revit 图元的最简单方法。可以选择完整的模型图元或其一部分拓扑（如面或边）。这会保持与该 Revit 对象的动态链接，因此，当 Revit 文件更新其位置或参数时，参照的 Dynamo 图元将在图形中更新。
...
> 1. 由于我们使用的是基本设置，因此我们选择“类别”下拉节点中的 _“体量”_ 来选择建筑体量。这可在“Revit”>“选择”选项卡中找到。
> 2. “体量”类别的输出只是类别本身。我们需要选择相应图元。为此，我们使用 _“所有类别图元”_ 节点。
...
以下是另一种方法。在这种情况下，我们将不会通过 Revit 层次结构 _（“所有类别图元”）_ 进行选择，而是改为在 Revit 中明确选择几何图形。

![](<images/2/selecting - exercise 04.jpg>)

> 1. 使用 _“Select Model Element”_ 节点，单击*“选择”*（或 _“更改”_ ）按钮。在 Revit 视口中，选择所需的图元。在本例中，我们选择的是建筑体量。
...
> 4. 使用 _“List.GetItemAtIndex”_，我们为_“index”_输入列表和*整数滑块*。在选定内容之间滑动，当到达 _“索引 9”_ 并隔离将桁架用作主要外立面时，便会停止。
...
假定我们要隔离建筑的主要外立面墙。我们可以使用 _“Select Faces”_ 节点来执行此操作。单击“选择”按钮，然后在 Revit 中选择四个主要外立面。
...
> 3. 我们可以使用 _“Watch3D”_ 节点放大几何图形（如果在“Watch 3D”中看不到梁，请单击鼠标右键，然后点击“缩放匹配”）。
