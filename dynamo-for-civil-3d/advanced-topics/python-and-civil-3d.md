...
{% hint style="warning" %} En règle générale, il est plus sûr d’obtenir l’ID de l’objet à l’aide de la propriété `InternalObjectId`, puis d’accéder à l’objet enveloppé dans une transaction. En effet, la propriété `InternalDBObject` renvoie un objet DBObject AutoCAD qui n’est pas accessible en écriture. 
{% endhint %} 
...
{% hint style="warning" %} En règle générale, il est recommandé d’inclure l’essentiel de la logique de votre script dans une transaction. Cela garantit un accès sécurisé aux objets que votre script lit/écrit. Dans de nombreux cas, l’omission d’une transaction peut provoquer une erreur irrécupérable. 
{% endhint %} 
