...
{% hint style="info" %} 각 Dynamo 라이브러리에서 액세스 가능한 리스트를 보려면 [스크립팅 참조](3-scripting-reference.md)를 참조하십시오. {% endhint %}
...
> [Python 노드](../8_coding_in_dynamo/8-3_python/1-python.md) 내 예시의 코드 "모듈"은 다음과 같습니다.
>
> 1. 입력
> 2. 스크립트 내부의 변수
> 3. 이러한 입력 및 변수를 사용하여 해당 함수를 수행하는 루프

{% hint style="info" %} 팁: 솔루션에서처럼 과정에 최대한 중점을 두십시오. {% endhint %}
...
{% hint style="info" %} 팁: 스크립트에서 엔터티(위 예에서는 상수)를 복제하기 전에 소스에 링크할 수 있는지에 대해 자문해 보십시오. {% endhint %}
...
{% hint style="info" %} 팁: 항상 사용자가 자신에게 표시된 모든 입력 값의 모든 조합을 사용한다고 가정하십시오. 그러면 원치 않는 갑작스러운 상황이 발생하지 않습니다. {% endhint %}
...
> [Python 노드](../8_coding_in_dynamo/8-3_python/1-python.md)에서의 예시 파일 디버깅은 다음과 같습니다.
>
