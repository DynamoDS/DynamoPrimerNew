...
{% hint style="info" %} 請參閱[指令碼撰寫參考](3-scripting-reference.md)以瞭解每個 Dynamo 資源庫可讓您存取的功能清單。
{% endhint %}
...
{% hint style="info" %} 秘訣：強調程序，如同您強調解決方案一樣。
{% endhint %}
...
{% hint style="info" %} 秘訣：在指令碼中複製實體前 (例如上述範例中的常數)，先檢查是否可以改為連結至來源。
{% endhint %}
...
{% hint style="info" %} 秘訣：永遠假定使用者將使用他/她會看到的每一個輸入值的每個組合。這樣有助於避免不必要的意外。
{% endhint %}
...
> 在 [Python 節點](../8\_coding\_in\_dynamo/8-3\_python/1-python.md)的範例檔案中除錯。
>
